...
public class Changelog implements BatchExtension {
  ...
  public Changelog(final ScmConfiguration conf, final SonarScmRepository repositoryBuilder, final ScmManager manager) {
    this.conf = conf;
    this.repositoryBuilder = repositoryBuilder;
    this.manager = manager;
  }

  public ProjectStatus load(final ProjectStatus status, final String startRevision) {
  ...
  private List<ChangeSet> retrieveChangeSets(String startRevision) {
    ScmRepository repository = this.repositoryBuilder.getScmRepository();
    ...
    try {
      ChangeLogScmResult result = this.manager.changeLog(repository, new ScmFileSet(this.conf.getBaseDir()),
      ...
      if (this.conf.isVerbose()) {
        File output = new File(this.conf.getWorkdir(), "scm_changelog.xml");
   ...
   */
  static boolean fixChangeSet(final ChangeSet changeSet) {
